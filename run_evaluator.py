...
def main(args):
    ...
    if args.sacrebleu:
        case_sensitive_scorer = bleu.SacrebleuScorer()
            ...
            for i, sample_id in enumerate(sample['id'].tolist()):
                ...
                for i, hypo in enumerate(hypos[i][:min(len(hypos), args.nbest)]):
                    ...
                    if has_target and i == 0:
                        ...
                        if hasattr(case_sensitive_scorer, 'add_string'):
                            case_sensitive_scorer.add_string(target_str, hypo_str)
    ...
    if has_target:
        print('| Generate {} with beam={} (Case-Sensitive): {}'.format(args.gen_subset, args.beam,
                                                                       case_sensitive_scorer.result_string()))
        print('| Generate {} with beam={} (Case-Insensitive): {}'.format(args.gen_subset, args.beam,
                                                                         case_insensitive_scorer.result_string()))

    return case_sensitive_scorer, case_insensitive_scorer
